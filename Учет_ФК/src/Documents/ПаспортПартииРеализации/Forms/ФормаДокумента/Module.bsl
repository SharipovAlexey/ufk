...
Процедура ПроизвестиРасчетНаКлиенте()
	Если Объект.Продукция.Количество() <> 0 и Объект.Продукция.Итог("ВесНеттоВлажный") <> 0 Тогда 
		Объект.Влажность 		= Окр(((Объект.Лоты.Итог("ВесНеттоВлажный") - Объект.Лоты.Итог("ВесНеттоСухой"))/Объект.Лоты.Итог("ВесНеттоВлажный")) *100,5,РежимОкругления.Окр15как20);
...
&НаСервере
Процедура ЛотыПриИзмененииНаСервере()
	Объект.Продукция.Очистить();
	Для каждого СтрокаЛотов из Объект.Лоты Цикл 
		Если ЗначениеЗаполнено(СтрокаЛотов.Лот)	Тогда 
			Для каждого СтрокаЛота из СтрокаЛотов.Лот.Продукция Цикл 
				СтруктураДанных  = Новый Структура();
				СтруктураДанных.Вставить("ВыбранноеЗначение",СтрокаЛота.ТранспортировочноеМесто);
				ДобавитьСтрокуНаСервере(СтруктураДанных);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ЛотыПриИзменении(Элемент)
	ЛотыПриИзмененииНаСервере();
	ПроизвестиРасчетНаКлиенте();
КонецПроцедуры

&НаСервере
Процедура ЛотыЛотПриИзмененииНаСервере(СтруктураДляПередачи)
	Если ЗначениеЗаполнено(СтруктураДляПередачи.ВыбранныйЛот) Тогда 
		СтруктураДляПередачи.ВесБруттоВлажный = Окр(СтруктураДляПередачи.ВыбранныйЛот.Продукция.Итог("ВесБруттоВлажный"),0,РежимОкругления.Окр15как20);
		СтруктураДляПередачи.ВесНеттоВлажный = Окр(СтруктураДляПередачи.ВыбранныйЛот.Продукция.Итог("ВесНеттоВлажный"),0,РежимОкругления.Окр15как20);
		СтруктураДляПередачи.ВесНеттоСухой = Окр(СтруктураДляПередачи.ВыбранныйЛот.Продукция.Итог("ВесНеттоСухой"),0,РежимОкругления.Окр15как20);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЛотыЛотПриИзменении(Элемент)
	текущаяСтрока = Элементы.Лоты.ТекущиеДанные;	
	
	СтруктураДляПередачи = новый Структура();
	СтруктураДляПередачи.Вставить("ВыбранныйЛот", текущаяСтрока.лот);
	СтруктураДляПередачи.Вставить("ВесБруттоВлажный", 0);
	СтруктураДляПередачи.Вставить("ВесНеттоВлажный", 0);
	СтруктураДляПередачи.Вставить("ВесНеттоСухой", 0);
	
	ЛотыЛотПриИзмененииНаСервере(СтруктураДляПередачи);
	ЗаполнитьЗначенияСвойств(текущаяСтрока,СтруктураДляПередачи,"ВесБруттоВлажный,ВесНеттоВлажный,ВесНеттоСухой")	
КонецПроцедуры
